...
//5.begin和end宏也是很重要的：外部调用基本上都是用的它，begin和end函数设置引用计数解决多线程的思想很强，需要根据引用计数的有无来决定时间的开始与结束，然后还有递归问题，引用计数就有优势了
...
class PerformanceProfiler: public Singleton<PerformanceProfiler>//为什么要用单例？因为有一个成员变量map可以把不同的函数给插入到map中去，而不用多创建类，所以单例就行了
{
